...
export default function Sidebar({ isOpen, onAddTask, onOpenSettings }) {
      ...
      useEffect(() => {
            const handleKeyDown = (e) => {
                ...
                if ((e.metaKey || e.ctrlKey) && e.key === 'k') {
                    e.preventDefault(); // Prevents the browser's default search bar from focusing
    ...
    const renderNoteItem = (note, prefix = 'note') => {
        ...
        <>
            <aside className={`${isOpen ? 'w-60' : 'w-0'} bg-[#F7F7F5] dark:bg-[#202020] border-r border-[#E9E9E7] dark:border-gray-400 shrink-0 transition-all duration-300 ease-in-out overflow-hidden flex flex-col h-full`}>
                ...
                <div className="flex-1 overflow-y-auto px-2 py-2 space-y-0.5 custom-scrollbar">
                    ...
                    {favoriteNotes.map(note => renderNoteItem(note, 'fav'))}
                    ...
                    {sharedNotes.length > 0 && (
                        <>
                            ...
                            {sharedNotes.map(note => renderNoteItem(note, 'shared'))}
                        </>
                    )}
                    ...
                    {privateNotes.map(note => renderNoteItem(note, 'private'))}
                </div>
