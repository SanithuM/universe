...
const Calendar = () => {
  ...
  return (
    <div className="h-screen flex flex-col bg-white font-sans text-[#37352f] overflow-hidden selection:bg-[#cce9ff]">

      {/* === HEADER === */}
      <header className="px-4 py-3 flex justify-between items-center bg-white shrink-0 border-b border-transparent hover:border-gray-100 transition-colors z-20">
        <div className="flex items-center gap-6">
          <div className="flex items-center gap-2">
            <button onClick={() => navigate('/app')} className="p-2 text-gray-400 hover:text-gray-800 hover:bg-gray-100 rounded-full transition mr-2" title="Back to Dashboard">
          ...
          <div className="flex items-center gap-1">
            <button onClick={() => setCurrentMonth(subMonths(currentMonth, 1))} className="p-1 hover:bg-gray-100 rounded text-gray-500"><ChevronLeftIcon className="w-5 h-5" /></button>
            <button onClick={() => setCurrentMonth(addMonths(currentMonth, 1))} className="p-1 hover:bg-gray-100 rounded text-gray-500"><ChevronRightIcon className="w-5 h-5" /></button>
            <button
              onClick={() => { setSelectedDate(new Date()); setCurrentMonth(new Date()); }}
              className="ml-2 px-3 py-1 text-sm font-medium border border-gray-200 rounded hover:bg-gray-50 text-gray-700 transition"
            >
              Today
            </button>
          </div>
        ...
        <div className="flex items-center gap-4">
          <Menu as="div" className="relative">
            <Menu.Button className="px-3 py-1.5 text-sm font-medium bg-gray-50 hover:bg-gray-100 rounded border border-gray-200 flex items-center gap-2 transition">
              <span className="capitalize">{viewMode === '3day' ? '3 Days' : viewMode}</span>
              <ChevronDownIcon className="w-4 h-4 text-gray-500" />
            </Menu.Button>
            <Transition as={Fragment} enter="transition ease-out duration-100" enterFrom="opacity-0 scale-95" enterTo="opacity-100 scale-100" leave="transition ease-in duration-75" leaveFrom="opacity-100 scale-100" leaveTo="opacity-0 scale-95">
              <Menu.Items className="absolute right-0 mt-1 w-32 bg-white rounded-lg shadow-lg border border-gray-100 focus:outline-none py-1 z-50">
          ...
          <div className="w-8 h-8 rounded-full bg-gray-100 border border-gray-200 overflow-hidden text-gray-500 flex items-center justify-center cursor-pointer hover:ring-2 ring-gray-100 transition">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <aside className="w-[260px] flex-shrink-0 bg-[#F7F7F5] flex flex-col border-r border-gray-200 hidden md:flex">
          ...
          <div className="flex-1 overflow-y-auto px-4 py-2 space-y-6">
            ...
            <div className="space-y-2">
              <p className="text-[11px] font-semibold text-gray-500 uppercase tracking-wider pl-1">Other</p>
        ...
        <aside className="w-[300px] flex-shrink-0 bg-white border-l border-gray-200 hidden lg:flex flex-col">
          ...
          <div className="p-4 pb-2 border-b border-transparent focus-within:border-gray-100 transition">
            <div className="relative group">
              <MagnifyingGlassIcon className="w-4 h-4 text-gray-400 absolute left-3 top-2.5 group-hover:text-gray-600 transition" />
              <input
                type="text"
                placeholder="Find anything..."
                className="w-full bg-[#F7F7F5] border-none rounded-md py-2 pl-9 pr-4 text-sm focus:ring-1 focus:ring-gray-300 placeholder-gray-400 transition"
              />
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto">
            {!selectedEvent ? (
              <div className="flex flex-col items-center justify-center h-64 text-center px-6">
                ...
                <p className="text-xs text-gray-500 mt-1">Select an event on the calendar to see details or edit.</p>
              </div>
            ) : (
              <div className="p-5 animate-in fade-in duration-200">
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div
                      ...
                      className={`w-5 h-5 rounded border cursor-pointer flex items-center justify-center transition
                                    ${selectedEvent.isCompleted ? 'bg-green-500 border-green-500' : 'bg-white border-gray-300 hover:border-gray-400'}`}
                    >
                  ...
                  <div className="flex gap-1">
                    <button onClick={() => openEditModal(selectedEvent)} className="p-1.5 text-gray-400 hover:text-gray-700 hover:bg-gray-100 rounded transition"><PencilIcon className="w-4 h-4" /></button>
                    <button onClick={() => handleDeleteEvent(selectedEvent._id)} className="p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded transition"><TrashIcon className="w-4 h-4" /></button>
                  </div>
                </div>

                <h2 className={`text-xl font-bold leading-tight mb-2 ${selectedEvent.isCompleted ? 'line-through text-gray-400' : 'text-gray-900'}`}>
                  {selectedEvent.title}
                </h2>

                <div className="text-sm text-gray-600 mb-6 flex flex-col gap-1">
                ...
                {selectedEvent.description && (
                  <div className="mb-6">
                    ...
                    <p className="text-sm text-gray-800 leading-relaxed whitespace-pre-wrap">{selectedEvent.description}</p>
                  </div>
                )}

                {selectedEvent.type === 'Meeting' && (selectedEvent.participants?.length > 0 || selectedEvent.participantEmail) && (
                  <div className="mb-6">
                    <h3 className="text-xs font-semibold text-gray-500 uppercase mb-3 text-purple-600">Participants</h3>
                    ...
                    {selectedEvent.participants && selectedEvent.participants.length > 0 ? (
                        ...
                        {selectedEvent.participants.map((p, i) => (
                          <div key={p._id || i} className="flex items-center gap-3 p-2 hover:bg-gray-50 rounded-lg transition-colors border border-transparent hover:border-gray-100">
                            ...
                            <div className="flex flex-col min-w-0">
                              <span className="text-sm font-medium text-gray-900 truncate">{p.username || "Guest User"}</span>
                      ...
                      <div className="flex items-center gap-3 p-2 rounded-lg bg-gray-50 border border-gray-100">
                        ...
                        <span className="text-sm text-gray-700 truncate">{selectedEvent.participantEmail}</span>
                      </div>
                    )}
                  </div>
                )}
...
const MonthView = ({ currentMonth, getItemsForDay, selectedDate, onEventClick }) => {
  ...
  return (
    <div className="h-full grid grid-cols-7 grid-rows-5 border-l border-gray-200 bg-gray-200 gap-px">
      {days.slice(0, 35).map((day, i) => {
        const items = getItemsForDay(day);
        const isCurrentMonth = isSameMonth(day, currentMonth);
        return (
          <div key={i} className={`bg-white p-2 flex flex-col gap-1 transition-colors hover:bg-gray-50 cursor-default ${!isCurrentMonth ? 'bg-gray-50' : ''}`}>
            <div className={`text-right text-xs font-semibold mb-1 ${isToday(day) ? 'text-red-500' : 'text-gray-500'}`}>
              {format(day, 'd')}
            </div>
              ...
              <div key={idx}
                ...
                className={`text-[10px] px-1.5 py-0.5 rounded truncate font-medium cursor-pointer transition
                    ...
                    item.type === 'Meeting' ? 'bg-purple-100 text-purple-700 hover:bg-purple-200' :
                      item.isAssignment ? 'bg-red-100 text-red-700 hover:bg-red-200' : 'bg-blue-100 text-blue-700 hover:bg-blue-200'}`}>
                {item.title}
              </div>
            ))}
          </div>
...
const TimeGridView = ({ viewMode, selectedDate, getItemsForDay, onSlotClick, onEventClick }) => {
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden bg-white">
      {/* Grid Header */}
      <div className="flex border-b border-gray-200 bg-white ml-14 shadow-[0_4px_6px_-6px_rgba(0,0,0,0.1)] z-10">
        {daysToShow.map((day, i) => (
          <div key={i} className="flex-1 text-center py-3 border-l first:border-l-0 border-gray-100 transition hover:bg-gray-50 cursor-pointer">
            <div className={`text-xs uppercase font-medium mb-1 ${isToday(day) ? 'text-red-500' : 'text-gray-500'}`}>{format(day, 'EEE')}</div>
            <div className={`text-xl font-normal w-8 h-8 flex items-center justify-center mx-auto rounded-full ${isToday(day) ? 'bg-red-500 text-white shadow-sm' : 'text-gray-800'}`}>
              {format(day, 'd')}
            </div>
          </div>
        ))}
      ...
      <div className="flex-1 overflow-y-auto relative custom-scrollbar flex">
        {/* Time Axis Sidebar */}
        <div className="w-14 flex-shrink-0 bg-white right-0 text-right pr-2 select-none border-r border-gray-100" style={{ height: TOTAL_HEIGHT }}>
          {hours.map(h => (
            <div key={h} className="relative text-xs text-gray-400 font-medium" style={{ height: HOUR_HEIGHT }}>
              ...
              {h !== 0 && (
                <span className="absolute -top-2.5 right-2 bg-white px-1 z-10">
        ...
        <div className="flex flex-1 relative" style={{ height: TOTAL_HEIGHT }}>
          ...
          <div className="absolute inset-0 pointer-events-none z-0 flex flex-col">
            {hours.map(h => (
              <div key={h} className="w-full border-t border-gray-100/50" style={{ height: HOUR_HEIGHT }}></div>
            ))}
          </div>
          ...
          {daysToShow.map((day, dayIndex) => {
            const items = getItemsForDay(day);
            return (
              <div key={dayIndex} className="flex-1 relative border-l border-gray-100/50 z-10 h-full" onClick={() => onSlotClick(day)}>
                {/* Events */}
                {items.map((item, idx) => {
                    ...
                    <div key={idx}
                      className={`absolute inset-x-1 rounded px-2 py-1 text-xs border-l-2 shadow-sm overflow-hidden z-20 cursor-pointer transition-all hover:brightness-95 group
                          ...
                          item.type === 'Meeting' ? 'bg-purple-50 border-purple-500 text-purple-900' :
                            item.isAssignment ? 'bg-red-50 border-red-500 text-red-900' : 'bg-blue-50 border-blue-500 text-blue-900'}`}
